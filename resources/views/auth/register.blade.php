    ...
    <div class="section-store-auth" data-aos="fade-up">
      <div class="container">
        <div class="row align-item center justify-content-center row-login">
          <div class="col-lg-4">
            <h2>
              Mulai untuk jual beli <br/> dengan cara terbaru
            </h2>
            <form method="POST" action="{{ route('register') }}">
              ...
              <div class="form-group">
                ...
                <input id="email" v-model="email" @change="checkForEmailAvailability()" type="email" class="form-control @error('email') is-invalid @enderror" :class="{ 'is-invalid' : this.email_unavailabel }" name="email" value="{{ old('email') }}" required autocomplete="email">
                @error('email')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
              ...
              <div class="form-group" v-if="is_store_open">
                ...
                v-model="store_name"
                id="store_name"
                class="form-control @error('password_confirm') is-invali @enderror"
                name="store-name"
                required
...
@push('addon-script')
    ...
    <script>
      ...
      var register = new Vue({
        ...
        methods: {
            checkForEmailAvailibility: function() {
                var self = this;
                ...
                .then(function (response) {
                    ...
                    if(response.data == 'Available') {
                        self.$toasted.show(
                            "Email anda tersedia! Silahkan lanjut langkat selanjutnya!",
                            {
                            position: "top-center",
                            className: "rounded",
                            duration: 1000,
